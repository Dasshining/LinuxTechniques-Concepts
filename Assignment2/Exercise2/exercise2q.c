...
{                                                 \
node_t *_oneHopeNode = NULL;			  \
unsigned int i = 0, j = 0;			  \
for(;node->nbrs[i] != NULL; i++){    	    	  \
    _oneHopeNode = node->nbrs[i]; 		    	  \
    for(j = 0; _oneHopeNode->nbrs[j] != NULL; j++){	  \
    	if(_oneHopeNode->nbrs[j] == node) j++;	  \
    	nbrnode = _oneHopeNode->nbrs[j];		  \
